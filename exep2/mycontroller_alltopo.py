...
rom p4runtime_lib.error_utils import printGrpcError
...
def writeTunnelRules(p4info_helper, ingress_sw, egress_sw, tunnel_id,
    ...
    forward_port=None
    if tunnel_id==100:  #s1到s2
        forward_port=2  #s1到s2的出端口是2端口
    if tunnel_id==200:  #s2到s1
        forward_port=2
    if tunnel_id==300:  #s1到s3
        forward_port=3
    if tunnel_id==400:  #s3到s1
        forward_port=2
    if tunnel_id==500:  #s2到s3
        forward_port=3
    if tunnel_id==600:  #s3到s2
        forward_port=3
    ...
    table_entry = p4info_helper.buildTableEntry(
        ...
        action_params={
            "port": forward_port  #不是之前固定的2号端口了
        })
    ingress_sw.WriteTableEntry(table_entry)
...
def main(p4info_file_path, bmv2_file_path):
    ...
    try:
        ...
        s3.SetForwardingPipelineConfig(p4info=p4info_helper.p4info,
							   		   bmv2_json_file_path=bmv2_fi le_path) 
